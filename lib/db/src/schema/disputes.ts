...
export const disputesTable = pgTable("disputes", {
  ...
  refCode:          text("ref_code").unique(),             // dispute_uid — e.g. "DSP-1234"
  orderId:          integer("order_id").references(() => ordersTable.id),
  orderNumber:      text("order_number"),
  customerId:       integer("customer_id").references(() => usersTable.id),   // the customer who raised the dispute
  supplierId:       integer("supplier_id").references(() => restaurantsTable.id), // the restaurant being disputed
