...
- Payments and wallet
- Admin CRM with fully live DB data across all tabs: Dashboard (6 real stats), **Restaurants** (real `/api/restaurants` data), **Users** (real `/api/admin/users` with points/level), **Bookings** (real `/api/admin/bookings` with restaurant + user names), **Reviews** (real `/api/reviews` data with user avatars, expert flag), **Registrations** (approve/reject partner applications), **Contracts** (create/activate/suspend + payment models & commission %), **Finance** (transaction ledger, invoice list), **Messages** (send messages to restaurants), **Offers** (approve/reject/revision workflow with commission override), **Modules** (toggle platform features via DB)
