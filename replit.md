...
- **AI Recommendations section on Homepage**: between Tabaq Stars and Curated Collections — calls `GET /api/recommendations` (GPT-powered, with city filter, fallback to top-rated); shows 3 restaurant cards with sparkle badge and bilingual AI-generated reason quotes. Cached with React Query (staleTime: 10 min, gcTime: 15 min) to avoid repeated OpenAI calls
